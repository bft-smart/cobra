...
int initialize_kzg(int t, bn_t alpha_input) {
	...
	commitment_bases = malloc(sizeof(ep_t) * (t + 1));
	for (int i = 0; i <= t; i++) {
	...
	witness_bases = malloc(sizeof(ep_t) * t);
	for (int i = 0; i < t; i++) {
...
static int compute_quotient_coefficients_dl(bn_t *secret_coefficients, bn_t x, bn_t *witness_scalars) {
    if (secret_coefficients == NULL || x == NULL || witness_scalars == NULL) {
        return -1;
    }
...
int compute_witnesses_dl(bn_t *secret_coefficients, bn_t *shareholders, int n_shareholders,
    ep_t *witnesses) {
    ...
    bn_t *witness_scalars = malloc(sizeof(bn_t) * threshold);
    ...
    for (int i = 0; i < n_shareholders; i++) {
        int status = compute_quotient_coefficients_dl(secret_coefficients, shareholders[i], witness_scalars);
        ...
        ep_mul_sim_lot(witnesses[i], witness_bases, witness_scalars, threshold);
    }
...
int commit_and_create_witnesses_dl(bn_t *secret_coefficients,
	...
	if (secret_coefficients == NULL || shareholders == NULL || commitment == NULL || witnesses == NULL) {
		return -1;
	}

	bn_t *commitment_scalars = malloc(sizeof(bn_t) * (threshold + 1));
	...
	ep_mul_sim_lot(commitment, commitment_bases, commitment_scalars, (threshold + 1));

	//compute witnesses
	int status = compute_witnesses_dl(secret_coefficients, shareholders, n_shareholders, witnesses);
	if (status != 0) {
		for (int i = 0; i <= threshold; i++) {
	...
	if (n_additional_shareholders > 0) {
		status = compute_witnesses_dl(secret_coefficients, additional_shareholders, n_additional_shareholders, additional_witnesses);
		if (status != 0) {
			for (int i = 0; i <= threshold; i++) {
				bn_free(commitment_scalars[i]);
			}
...
int jna_commit_and_create_witnesses_dl(uint8_t *secret_coefficients,
	...
	if (secret_coefficients == NULL || shareholders == NULL
	...
	int status = commit_and_create_witnesses_dl(secret_coeffs_bn, shareholders_bn, n_shareholders,
...
int verify_share_dl(bn_t shareholder, bn_t secret_share, ep_t commitment, ep_t witness) {
    if (shareholder == NULL || secret_share == NULL || commitment == NULL || witness == NULL) {
        return -1;
    }
	...
	* rhs = e(g, g)^phi(i)
	*/
	fp12_exp(rhs, gPairing, secret_share);
...
int jna_verify_share_dl(uint8_t *shareholder, uint8_t *secret_share, uint8_t *commitment, uint8_t *witness) {
	if (shareholder == NULL || secret_share == NULL || commitment == NULL || witness == NULL) {
    	return -1;
    }
	bn_t shareholder_bn, secret_share_bn;
	...
	int valid = verify_share_dl(shareholder_bn, secret_share_bn, commitment_ep, witness_ep);
...
int jna_verify_same_secret_evaluation_at_dl(uint8_t *x, int n_commitments, uint8_t *commitments, uint8_t *witnesses) {
	if (x == NULL || n_commitments < 0 ||
		(n_commitments > 0 && (commitments == NULL || witnesses == NULL))) {
		return -1;
	}
	...
	for (int i = 1; i < n_commitments; i++) {
		...
		/*
		* Check:
		*   e(C_i, g2) * e(-W_i, alpha_minus_x_g2)
		* =
		*   e(C_0, g2) * e(-W_0, alpha_minus_x_g2)
		*
		* Equivalent to:
		*   e(C_i - C_0, g2) * e(W_0 - W_i, alpha_minus_x_g2) == 1
		*/
