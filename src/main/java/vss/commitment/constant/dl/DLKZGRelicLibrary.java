...
public interface DLKZGRelicLibrary extends Library {
	DLKZGRelicLibrary INSTANCE = Native.load("dl_kzg_commitments", DLKZGRelicLibrary.class);
	...
	int jna_g1_generator_1(byte[] result);

	int jna_commit_and_create_witnesses_dl(byte[] secret_coefficients,
	...
	int jna_verify_share_dl(byte[] shareholder, byte[] secret_share, byte[] commitment, byte[] witness);
	...
	int jna_verify_same_secret_evaluation_at_dl(byte[] x, int n_commitments, byte[] commitments, byte[] witnesses);
}
