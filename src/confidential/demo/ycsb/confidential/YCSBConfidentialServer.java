...
public class YCSBConfidentialServer implements ConfidentialSingleExecutable {
    ...
    public static void main(String[] args) {
        if (args.length == 1) {
            new YCSBConfidentialServer(Integer.parseInt(args[0]));
        } else {
    ...
    public ConfidentialMessage appExecuteOrdered(byte[] plainData, VerifiableShare[] shares, MessageContext msgCtx) {
        ...
        switch (aRequest.getType()) {
            case CREATE: { // ##### operation: create #####
                // Only create records
                if (aRequest.getEntity() == YCSBConfidentialMessage.Entity.RECORD) { // ##### entity: record #####
    ...
    public ConfidentialMessage appExecuteUnordered(byte[] plainData, VerifiableShare[] shares, MessageContext msgCtx) {
        ...
        VerifiableShare[] sharesResponse = null;
        if (aRequest.getType() == YCSBConfidentialMessage.Type.READ) {// ##### operation: read #####
            switch (aRequest.getEntity()) {
                case RECORD: // ##### entity: record #####
                    ...
                    if (!mTables.get(aRequest.getTable()).containsKey(aRequest.getKey())) {
                        ...
                        Map<String, VerifiableShare> response = mTables.get(aRequest.getTable()).get(aRequest.getKey());
                        String[] keys = new String[response.size()];
                        sharesResponse = new VerifiableShare[response.size()];
                        int k = 0;
                        for (Map.Entry<String, VerifiableShare> entry : response.entrySet()) {
                            keys[k] = entry.getKey();
